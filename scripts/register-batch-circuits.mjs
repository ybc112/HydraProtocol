 ...
 * 2. 运行: node register-batch-circuits.mjs
 */
...
async function main() {
  ...
  for (const circuit of CIRCUITS) {
    ...
    try {
      ...
      // 参数：registry, circuit_name, curve_type, vk_data, description, clock, ctx
      tx.moveCall({
        target: `${PACKAGE_ID}::zkp_verifier::register_circuit`,
        arguments: [
          tx.object(ZKP_REGISTRY_ID),           // registry
          tx.pure.string(circuit.name),          // circuit_name
          tx.pure.u8(1),                         // curve_type (1 = BN254)
          tx.pure.vector('u8', Array.from(vkBytes)), // vk_data
          tx.pure.string(circuit.displayName),   // description
